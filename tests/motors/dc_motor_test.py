...
class TestDcMotor(unittest.TestCase):
    ...
    @patch('motors.dc_motor.Motor')  
    @patch('motors.dc_motor.PWMOutputDevice')
    def setUp(self, mock_pwm_class, mock_motor_class):
        self.mock_motor_instance = MagicMock()
        self.mock_pwm_instance = MagicMock()
    
        mock_motor_class.return_value = self.mock_motor_instance
        mock_pwm_class.return_value = self.mock_pwm_instance

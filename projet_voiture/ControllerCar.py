...
class ControllerCar:
    ...
    QUOI: Surveille les mesures de distance fournies par les capteurs (via CapteurDistance),
          commande les moteurs et le servo pour éviter les obstacles, et simule la vitesse actuelle.
    """
    _instance = None
    ...
    def __init__(self):
        # Eviter la réinitialisation multiple (Singleton)
        ...
        self.current_speed = 0.0          # Vitesse initiale
        self.max_speed = 2.0              # Vitesse maximum (m/s) à 100% de puissance
        self.acceleration_rate = 0.1      # Incrément de vitesse par cycle (m/s)
        self.deceleration_rate = 0.1      # Diminution de vitesse par cycle (m/s)
    ...
    def run(self):
        ...
        Simule une accélération progressive en l'absence d'obstacles et ajuste la vitesse en cas de manœuvres.
        """
        ...
        try:
            while True:
                # Simulation d'accélération progressive quand tout est normal
    ...
    def smooth_decelerate(self, target_speed):
        """
        Décélère progressivement la vitesse jusqu'à atteindre target_speed.
        """
    ...
    def smooth_accelerate(self, target_speed):
        """
        Accélère progressivement la vitesse jusqu'à atteindre target_speed.
        """
        while self.current_speed < target_speed:
            self.current_speed += self.acceleration_rate
            if self.current_speed > target_speed:
    ...
    def handle_left_obstacle(self):
        ...
        # Réduire la vitesse pour le virage
        self.smooth_decelerate(0.5)
        self.motor_ctrl.forward(80)  # On avance à une puissance réduite

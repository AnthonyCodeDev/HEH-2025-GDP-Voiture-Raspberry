...
class ControllerCar:
    ...
    def __init__(self):
        ...
        self.deceleration = 0.2      # Décélération en cas d'interruption #what??????????????????????????????????????????????????????
    ...
    def mode_course(self):
        ...
        #while infrared.get_status() != 1: # Check constantly if black line not detected 
            ...
            self.ultrasound_forward.stop_thread()  # Thread for front obstacle detection stopped
            self.ultrasound_left.stop_thread()       # Thread for left distance measurements stopped
            self.ultrasound_right.stop_thread()      # Thread for right distance measurements stopped
